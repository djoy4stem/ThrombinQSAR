            ...
            try:
                ...
                if isinstance(model, (LGBMRegressor, LGBMClassifier)):
                    model.fit(
                        ...
                        # eval_set=[(val_df.drop(columns=[target_column], axis=1), val_df[target_column])],
                        # eval_metric='mae',
                        # callbacks=[early_stopping(50)],  ## to avoid overfitting
                    )
                ...
                print(f"Could not complete trial\n: {e}")
                scores.append(float("inf"))  ## penalize the invalid trial
